...
for i,z0 in enumerate(z):
    os.system("./fargo3d -o 'zmax={0:4.4f} outputdir=outputs/output{1:d} ny={2:d} nz={3:d} ninterm={4:d} dt={5:f}' setups/mri-growth-sb/mri-growth-sb.par".format(z0,i, ny, nz, ninterm, dt))
...
for j in range(len(z)):
    outputdir = "./outputs/mri-growth-sb/output{:d}/".format(j)
    ...
    for i in range(ntot//ninterm + 1):
        ...
        bz = fromfile(outputdir+"bz{:d}.dat".format(i)).reshape(nz,ny)
    ...
    ax2.plot(time,maxbz,'ro')
    print(mode, a)
plt.savefig(f"setups/mri-growth-sb/mri-growth-sb-modes.png", dpi=300)
