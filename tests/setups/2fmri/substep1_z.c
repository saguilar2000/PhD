...
//<#>
	...
	//ions
	if (Fluidtype == GAS) {
	  vz_temp[ll] += dt*ASPECTRATIO*OMEGAFRAME*OMEGAFRAME; // Constant gravitational acceleration
	}
	//neutrals
	if (Fluidtype == DUST) {
	  vz_temp[ll] -= dt*ASPECTRATIO*OMEGAFRAME*OMEGAFRAME*EPSILON; // Constant gravitational acceleration
	}	
#endif
